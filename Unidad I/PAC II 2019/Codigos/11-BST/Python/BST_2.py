...
class BinarySearchTree:
    ...
    def _delete(self,value):
        return self.deleteFromTree(value, self.root)

    def deleteFromTree(self, value, current):
        if (current.value == value):
            ...
            if (value > current.value):
                if (current.rightChild):
                    current = current.rightChild
                    return self.deleteFromTree(value, current)
        ...
        if current.value > value:
            if current.leftChild:
                return self.deleteFromTree(value, current.leftChild)
        elif current.value < value:
            if current.rightChild:
                return self.deleteFromTree(value, current.rightChild)
        if current.value == value:
            current = None
            return current
...
class LinkedList:
    ...
    def delete(self, value):
        ...
        if self.first is None:
            ...
            while (current.value.name != value.name and current.next is not None):
                prev = current
                current = current.next
...
class Graph:
    ...
    def delete_vertex(self, vertex_name):
        vertexToDelete = self.vertex.search(vertex_name)
        edgesList = vertexToDelete.edges
        edgesToDelete = edgesList.first

        while(edgesToDelete):
            edgesList.delete(edgesToDelete.value)
            edgesToDelete = edgesToDelete.next
            
        self.vertex.delete(vertexToDelete)
